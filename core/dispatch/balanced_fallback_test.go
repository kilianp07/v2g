...
func TestBalancedFallback_Reallocate(t *testing.T) {
	fb := NewBalancedFallback(logger.NopLogger{})
	vs := []model.Vehicle{
		{ID: "v1", MaxPower: 50, SoC: 1},
		{ID: "v2", MaxPower: 100, SoC: 1},
	}
	fb.SetVehicles(vs)
...
func TestBalancedFallback_SkipLowSoC(t *testing.T) {
	fb := NewBalancedFallback(logger.NopLogger{})
	vs := []model.Vehicle{
		{ID: "v1", MaxPower: 50, SoC: 1},
		{ID: "v2", MaxPower: 50, SoC: 0.2},
	}
	fb.SetVehicles(vs)
...
func TestBalancedFallback_NoCapacity(t *testing.T) {
	fb := NewBalancedFallback(logger.NopLogger{})
	vs := []model.Vehicle{
		{ID: "v1", MaxPower: 40, SoC: 1},
		{ID: "v2", MaxPower: 40, SoC: 1},
	}
	fb.SetVehicles(vs)
	current := map[string]float64{"v1": 40, "v2": 40}
	failed := []model.Vehicle{vs[0]}
	sig := model.FlexibilitySignal{PowerKW: 80}
...
func TestDispatchManager_BalancedFallback(t *testing.T) {
	vehicles := []model.Vehicle{
		{ID: "v1", SoC: 0.8, IsV2G: true, Available: true, MaxPower: 50},
		{ID: "v2", SoC: 0.8, IsV2G: true, Available: true, MaxPower: 50},
	}
